...
if(isset($_POST['email'])){
	...
	if($senha1==$senha2){
		...
		if($linhas==0){
			$sqlf = "insert into usuario(email,nome,data_nasc,senha,foto,nivel) values('$email','$nome','$data_nasc','$senha1','$foto','$nivel')";
			...
			if($insere){
				...
				if($nivel_cadastrando!='adm'){
					$sql = "select id_usuario,nome,email,foto,nivel from usuario where email='$email'";
